...
#        print("Odometry time stamp nsec = ",  self.messaggio_odometria.header.stamp.nanosec)
    ...
    def setup_robot_devices(self, properties):
        ...
        self.__camera = self.__robot.getDevice('TiagoCamera')
    ...
    def take_data_from_Webots_node(self,properties,webots_node):
        ...
        for k in range(self.n):
                    ...
                    if(nodo_livello_1.getTypeName() == 'Robot'):    # Cerco il nodo Door in base al nome
                        ...
                        if(robot_name.getSFString() == 'TIAGo Base'):
                            self.__node.get_logger().info('è stato trovato il nodo TIAGo Base')
                            ...
                            self.__node.get_logger().info('trovato rotation TIAGo Base: ' + str(rotation_oggetto.getSFRotation()))
                            
                            self.__node.get_logger().info('trovato translation TIAGo Base: ' + str(translation_oggetto .getSFVec3f()))
                            ...
                            self.__node.get_logger().info('Sono dentro TiagoCamera')
                            tiago_camera_rot = nodo_livello_10.getField('rotation') 
                            tiago_camera_tra = nodo_livello_10.getField('translation')   
                            self.__node.get_logger().info('rotation_TiagoCamera: ' + str(tiago_camera_rot.getSFRotation()))
                            self.__node.get_logger().info('translation_TiagoCamera: ' + str(tiago_camera_tra.getSFVec3f()))
